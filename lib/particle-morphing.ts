...
/**
 * Generates molecular target positions (H₂O molecule layout).
 * MolecularNetwork renders its own molecule internally;
 * these positions are used only for morph-target generation.
 */
export function generateMolecularTargets(): THREE.Vector3[] {
  return [
    new THREE.Vector3(0, 2.2, 0),      // Oxygen (center)
